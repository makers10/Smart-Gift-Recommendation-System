  ...
  return (
    <div className="min-h-screen flex flex-col" style={{background: "linear-gradient(160deg,#fff1f2 0%,#fdf2f8 50%,#fce7f3 100%)"}}>
      ...
      <header className="relative z-10 text-center pt-12 pb-6 px-4">
        ...
        <h1 className="font-serif text-4xl md:text-5xl font-bold text-rose-700 tracking-wide">
          Gift with Love
        </h1>
      ...
      {/* Nav Tabs */}
      <nav className="relative z-10 flex justify-center gap-3 mb-8 px-4">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => { setTab(t.id); setResult(null); }}
            className={`px-5 py-2 rounded-full text-sm font-medium tracking-wide transition-all cursor-pointer shadow-sm
              ${tab === t.id
